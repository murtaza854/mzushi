...
function CountryForm(props) {
    ...
    const [name, setName] = useState({ value: '', error: false, helperText: 'Enter a name Ex. Pakistan' });
    const [abbreviation, setAbbreviation] = useState({ value: '', error: false, helperText: 'Enter an abbreviation Ex. PK' })
    ...
    useEffect(() => {
        (
            async () => {
                if (id) {
                    const response = await fetch(`${api}/country/getById`, {
                        method: 'POST',
                        headers: {
                    ...
                    if (content.data) {
                        const { data } = content;
                        setName({ value: data.name, error: false, helperText: 'Enter a name Ex. Pakistan' });
                        setAbbreviation({ value: data.abbreviation, error: false, helperText: 'Enter an abbreviation Ex. PK' });
                        setCheckBoxes({ active: data.active });
                        setDisabled(false);
                    } else {
                        window.location.href = window.location.href.split('/admin')[0] + '/admin/country';
                    }
                }
            })();
    }, [id]);
    ...
    useEffect(() => {
        ...
        else if (abbreviation.error === true) flag = true;
        else if (abbreviation.value.length === 0) flag = true;
        else flag = false;
        setDisabled(flag);
    }, [name, abbreviation]);
    ...
    const handleNameChange = (event) => {
        if (event.target.value.length === 0) {
            ...
            setName({ value: event.target.value, error: true, helperText: 'Country already exists!' });
        } else {
            setName({ value: event.target.value, error: false, helperText: 'Enter a name Ex. Pakistan' });
        }
    }

    const handleAbbreviationChange = event => {
        if (event.target.value.length === 0) {
            setAbbreviation({ value: event.target.value, error: true, helperText: 'Abbreviation is required!' });
        } else {
            setAbbreviation({ value: event.target.value, error: false, helperText: 'Enter an abbreviation Ex. PK' });
        }
    }
        ...
        <Container fluid>
            ...
            <Form onSubmit={onSubmit}>
                ...
                <Row>
                    ...
                    <Col md={6}>
                        <FormControl variant="standard" fullWidth>
                            <InputLabel error={abbreviation.error} htmlFor="abbreviation">Abbreviation</InputLabel>
                            <Input id="abbreviation"
                                value={abbreviation.value}
                                onChange={handleAbbreviationChange}
                                onBlur={handleAbbreviationChange}
                                error={abbreviation.error}
                            />
                            <FormHelperText error={abbreviation.error}>{abbreviation.helperText}</FormHelperText>
                        </FormControl>
                    </Col>
                </Row>
